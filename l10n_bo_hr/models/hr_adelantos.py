...
class HrAdelantos(models.Model):
    _name = "hr.adelantos"
    _description = "Adelantados"

    def _get_journal(self):
        config_obj = self.env["hr.adelantos.config"]
        journal_id = False
    ...
    state_p = fields.Boolean('Descontar en Nomina', readonly=True, default=True, help="Descuento en planilla",
                             states={'draft': [('readonly', False)]})
    ...
    def cancel_adelantos(self):
        return self.write({'state': 'cancel'})

    def hr_process_adelantos(self):
        if self.assess:
            self.process_adv()
        if not self.state_p:
            self.write({'state': 'process'})
            return self.hr_done_adelantos()
        return self.write({'state': 'process'})

    def hr_done_adelantos(self):
        return self.write({'state': 'done'})


class HrAdelantosConfig(models.Model):
    _name = 'hr.adelantos.config'
...
class HrEmployee(models.Model):
    ...
    def _advance_count(self):
        ...
        for s in self:
            s.advance_count = adelantos.search_count([('employee_id', '=', s.id)])
